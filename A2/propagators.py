    ...
    for c in csp.get_all_cons():
        if newVar:
            if newVar not in c.get_scope():
                continue
        if c.get_n_unasgn() == 1:
            rem = c.get_unasgn_vars()[0] #last variable to be assigned
            rem_domain = rem.cur_domain()
                
            for d in rem_domain:
                if not c.has_support(rem,d):
                    rem.prune_value(d)
                    pruned.append((rem, d))

                    if rem.cur_domain_size()==0:
                        return False, pruned
    return True, pruned
...
def prop_GAC(csp, newVar=None):
    ...
    # TODO! IMPLEMENT THIS!
    if not newVar:
        GACQueue = set(csp.get_all_cons())
    else:
        GACQueue = set(csp.get_cons_with_var(newVar))
            ...
            for d in v.cur_domain():
                if not C.has_support(v,d):
                    ...
                    if v.cur_domain_size()==0:
                        ...
                        GACQueue = (GACQueue|set(csp.get_cons_with_var(v)))
