    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if (nextx,nexty) in self.corners:
                if (nextx,nexty) not in progress:
                    progress += ((nextx,nexty),) 
            
            
            if not self.walls[nextx][nexty]:
                cost = self.getCostOfActions(((action),))
                nextState = ((nextx, nexty),) + progress
                successors.append( ( nextState, action, cost) )            
...
def cornersHeuristic(state, problem):
    ...
    while len(corners) != 0:
        ...
        for i in corners:
            xy1 = cursor
            xy2 = i
            if (abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])) < best_cost:
                best_cost = (abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]))
                best_node = i
        total_cost += best_cost
        corners.remove(best_node)
        cursor = best_node
...
def foodHeuristic(state, problem):
    ...
    minx = foodGrid.asList()[0]
    maxx = foodGrid.asList()[0]
    miny = foodGrid.asList()[0]
    maxy = foodGrid.asList()[0]
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        steps = search.breadthFirstSearch(problem)
